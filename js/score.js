...
function mouseClick(click) {
  ...
  const distanceBetween = Math.sqrt(
    Math.pow(mouseClickPosition.x - targetPosition.x, 2) +
      Math.pow(mouseClickPosition.y - targetPosition.y, 2)
  );

  if (distanceBetween <= difficulty.targetRadius / 3) {
    updateScore(3);
  } else if (
    distanceBetween <= (difficulty.targetRadius / 3) * 2 &&
    distanceBetween > difficulty.targetRadius / 3
  ) {
    updateScore(2);
  } else if (
    distanceBetween <= difficulty.targetRadius &&
    distanceBetween > (difficulty.targetRadius / 3) * 2
  ) {
    updateScore(1);
  } else {
